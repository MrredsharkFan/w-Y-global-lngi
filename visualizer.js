const renderer = (() => {
    ...
    function draw(recalculate) {
        ...
        if (newConfig["STACKMODE"]) {
            ...
            for (var cycles = 0; cycles < 2; cycles++) {
                ...
                if (cycles) {
                    // High quality Retina/High-DPI Scale adjustment for non-stack view
    ...
    function getNodeRelationship(node, highlightIndex, m) {
        ...
        // Find the active node at the current cursor position
        let activeNode = findHighestWithPosition(m, highlightIndex);
        if (!activeNode) return "none";
        if (node.position === activeNode.position && node.coord.join(',') === activeNode.coord.join(',')) return "active";

        // 1. Check if 'node' is an ancestor of 'activeNode'
        let curr = activeNode;
        while (curr) {
            let p = parent(m, curr) || leftLeg(m, curr);
        ...
        // 2. Check if 'node' is a child/descendant of 'activeNode'
        curr = node;
        while (curr) {
            let p = parent(m, curr) || leftLeg(m, curr);
            if (p && activeNode.position === p.position && activeNode.coord.join(',') === p.coord.join(',')) {
                return "child";
            }
            curr = p;
        }
